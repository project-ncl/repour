...
@asyncio.coroutine
def get_version_from_pme_result(pme_result_json):
    """
    Format of JSON should be as follows:
    ...
    Function tries to extract version generated by PME from the result json file

    If for some reason (json file not parseable, json structure changed, etc),
    we can't process the file, the version returned will be None
    """
    try:
        data = json.loads(pme_result_json)
        version = data['VersioningState']['executionRootModified']['version']
        return version
